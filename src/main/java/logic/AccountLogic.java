...
public class AccountLogic extends GenericLogic<Account,AccountDAL>{
    ...
    public Account createEntity(Map<String, String[]> parameterMap) {
        ...
        String displayName = parameterMap.get(DISPLAY_NAME)[0];
        if (displayName == null || displayName.isEmpty())
            throw new ValidationException("Display Name is required");
         else if ((displayName.length() < 1) || (displayName.length() > 45))
            throw new ValidationException("Display Name should be between 1 and 45 characters");
        account.setDisplayName(parameterMap.get(DISPLAY_NAME)[0]);
        
        String user = parameterMap.get(USER)[0];
        if (user == null || user.isEmpty())
            throw new ValidationException("User is required");
        else if ((user.length() < 1) || (user.length()>45))
            throw new ValidationException("User should be between 1 and 45 characters");
