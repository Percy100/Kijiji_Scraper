        ...
        log("POST");
          ...
          try {
            ...
            if  (itLogic.getWithId(x) == null){
                Item item = itLogic.createEntity(request.getParameterMap());
                
                Map<String, String[]> imMap = new HashMap<>();
                imMap.put(ImageLogic.NAME, new String[]{});
                imMap.put(ImageLogic.URL, new String[]{"url"});
                imMap.put(ImageLogic.PATH, new String[]{System.getProperty("user.home")+"/KijijiImages/"+ 1 +".jpg"});
                Image im = imLogic.createEntity(imMap);
         
                imLogic.add(im);
                item.setImage(imLogic.getWithId(im.getId()));
                item.setCategory(caLogic.getWithId(1));
